...
const App: React.FC = () => {
  ...
  const handleSearch = async (address: string, forceAuth = false) => {
    // Check if user is logged in (or forceAuth is true from login callback)
    if (!state.user && !forceAuth) {
        setPendingAddress(address);
        // Redirect to Login page instead of just 'dashboard' auth gate
        setState(prev => ({ ...prev, view: 'login' })); 
        return;
    }

    setLoadingMessage('Connecting to Zillow Data Feed...');
    setIsLoading(true);
  ...
  const renderView = () => {
    ...
    if (state.view === 'landing') return <LandingPage onSearch={(addr) => handleSearch(addr)} onNavigate={(view) => setState(p => ({...p, view}))} isMember={false} />;
    ...
    <div className="min-h-screen bg-[#020617] text-slate-200 font-sans selection:bg-pink-500 selection:text-white">
      {isLoading && (
          <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-sm flex items-center justify-center flex-col">
              ...
              <p className="text-slate-400 text-sm">Secure Connection...</p>
          </div>
      )}
              ...
              <aside className="w-20 md:w-64 bg-slate-950 border-r border-white/5 flex-shrink-0 flex flex-col justify-between py-6 px-3">
                  <div>
                      <div className="flex items-center gap-3 px-2 mb-10">
                          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-600 to-pink-600 flex items-center justify-center font-bold text-white text-xl">W</div>
                  ...
                  <div className="border-t border-white/5 pt-6">
                      <button onClick={() => setState(p => ({...p, view: 'profile'}))} className="flex items-center gap-3 w-full p-2 hover:bg-white/5 rounded-xl transition-colors text-left group">
                          ...
                          <div className="hidden md:block overflow-hidden">
                              ...
                              <div className="text-[10px] text-emerald-400 font-bold uppercase">Premium</div>
                          </div>
                      </button>
                      ...
                      <div className="max-w-7xl mx-auto px-4">
                          <div className="flex items-center justify-between h-20">
                              <div 
                                  ...
                                  onClick={() => setState(p => ({...p, view: 'landing'}))}
                              >
